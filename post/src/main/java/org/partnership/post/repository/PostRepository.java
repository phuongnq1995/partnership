...
public interface PostRepository extends JpaRepository<Post, Long>{
	...
	@Query("select distinct p from Post p JOIN p.locations l where l.id = :locationId "  
		+ " and (lower(p.title) like %:keywords% or lower(p.company.name) like %:keywords%) and p.status = 1 " )
	...
	@Query("select p from Post p where (lower(p.title) like %:keywords% or lower(p.company.name) like %:keywords%) and p.status = 1 " )
	...
	@Query("select p from Post p INNER JOIN p.locations l where l.id = :locationId and p.status = 1 ")
	...
	@Query(value="select * from post p inner join ("
		...
		+"group by a.post_id ) b on b.a_post_id = p.id where p.status = 1 order by quantity desc ", nativeQuery=true)
